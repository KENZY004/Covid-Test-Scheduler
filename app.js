...
//     age: document.getElementById('patient-age').value,
//     riskLevel: document.querySelector('input[name="risk-level"]:checked').value,
//     hospital: 'city-general', // Fixed hospital value
//     hospitalName: 'City General Hospital', // Fixed hospital name
//     type: selectedService === 'covid-test' ? 'COVID-19 Test' : 'COVID-19 Vaccination',
//     time: selectedTime,
//     date: selectedDate,
//     bookedAt: new Date().toISOString(),
...
//     // Adding click handlers for service cards
//     document.getElementById('covid-test-card').addEventListener('click', function(e) {
//         e.preventDefault();
//         selectService('covid-test');
//     });
...
//     // Initialize slot manager will handle its own state
...
//     // Refresh time slots if on booking tab
//     if (selectedDate) {
//         generateTimeOptions();
//     }
...
function initializeApp() {
    ...
    // Initialize queues with existing appointments
    queueManager.testingQueue = new PriorityQueue();
    queueManager.vaccinationQueue = new PriorityQueue();
    queueManager.currentlyServing = { testing: null, vaccination: null };
    
    // Add all active appointments to their respective queues
    appointments.forEach(appointment => {
        if (!appointment.status || appointment.status !== 'cancelled') {
            const queueType = appointment.type.includes('Test') ? 'testing' : 'vaccination';
            queueManager.addToQueue(queueType, appointment);
    ...
    if (savedQueueState) {
        try {
            const queueState = JSON.parse(savedQueueState);
            if (queueState.currentlyServing) {
